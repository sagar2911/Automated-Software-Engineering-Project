...
//	}
	...
	public Div2(Tbl num, int i, int j, ColType type) {
		...
		for (int k = 0; k < num.rows.size(); k++) {
			...
			temp.add(num.rows.get(k).get(j));
			temp.add(num.rows.get(k).get(i));
	...
	private int divide(List<List<Cell>> c, Col num, int low, int high, int rank, String t) {
		...
		Tbl tblList = new Tbl();
		if (t == "Num") {
			leftList.addRow(1, Arrays.asList(new Cell("$y"), new Cell("$x")));
			rightList.addRow(1, Arrays.asList(new Cell("$y"), new Cell("$x")));
			tblList.addRow(1, Arrays.asList(new Cell("$y"), new Cell("$x")));
		} else {
...
//			}
		...
		if (cut != -1) {
			...
			if (t == "Num") {
				...
				rangeCols.add(tblList.cols.get(0));
				rangeCols.add(tblList.cols.get(1));
